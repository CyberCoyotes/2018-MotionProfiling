...
public class Robot extends IterativeRobot {
	...
	public void disabledInit() {
		...
		left.config_kF(0, 0.076, Constants.kTimeoutMs);
		left.config_kP(0, 2.000, Constants.kTimeoutMs);
		left.config_kI(0, 0.0, Constants.kTimeoutMs);
		left.config_kD(0, 20.0, Constants.kTimeoutMs);

		/* Our profile uses 10ms timing */
		left.configMotionProfileTrajectoryPeriod(10, Constants.kTimeoutMs); 
		/*
		...
		right.config_kF(0, 0.076, Constants.kTimeoutMs);
		right.config_kP(0, 2.000, Constants.kTimeoutMs);
		right.config_kI(0, 0.0, Constants.kTimeoutMs);
		right.config_kD(0, 20.0, Constants.kTimeoutMs);

		/* Our profile uses 10ms timing */
		right.configMotionProfileTrajectoryPeriod(10, Constants.kTimeoutMs); 
		/*
		 * status 10 provides the trajectory target for motion profile AND
		 * motion magic
		 */
	...
	public void teleopPeriodic() {
		...
		if (btns[5] == false) {
			...
			left.set(ControlMode.MotionProfile, setOutputL.value);
			right.set(ControlMode.MotionProfile, setOutputR.value);
